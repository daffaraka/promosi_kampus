...
class AkunController extends Controller
{
    ...
    public function ubahUser($id, Request $request)
    {
        $data['title'] = 'Data User';
    ...
    public function dataTable(Request $request)
    {
        ...
        if (!empty($akun_data)) {
            foreach ($akun_data as $akun_val) {
                $url = route('user.edit', ['id' => $akun_val->id]);
                $urlHapus = route('user.delete', $akun_val->id);
                ...
                $role =  Roles::where('id', '!=', Auth::id())
                    ->where('id', $akun_val->role)
                    ->get();

                $akunnestedData['name'] = $akun_val->name;
                $akunnestedData['email'] = $akun_val->email;
                $akunnestedData['role'] =  $role[0]->role;
